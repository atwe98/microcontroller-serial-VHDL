...
begin
	S <=    B when alufs=ALU_B else
             	std_logic_vector(signed(A)-signed(B)) when alufs=ALU_SUB else
             	std_logic_vector(signed(A)+signed(B)) when alufs=ALU_ADD else
		std_logic_vector(signed(B)+1) when alufs=ALU_B_INC else
...
signal p_reg : std_logic_vector(11 downto 0) := "000000000000";
  begin   
process (clk,raz)
begin
if rising_edge(clk) then
		if raz ='1' then p_reg <= (others =>'0');
		elsif load ='1' then p_reg <= data_in;
		end if;
end if;
end process;
data_out <= p_reg;
...
if rising_edge(clk) then
		if raz ='1' then interne <= (others =>'0');
				data_out <= (others =>'0');
		elsif load ='1' then interne <= data_in (15 downto 12);
